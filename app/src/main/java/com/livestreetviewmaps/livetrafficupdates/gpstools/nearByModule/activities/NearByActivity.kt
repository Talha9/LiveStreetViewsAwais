...
import com.livestreetviewmaps.livetrafficupdates.gpstools.Utils.fourSquareApi.ApiModel
import com.livestreetviewmaps.livetrafficupdates.gpstools.Utils.fourSquareApi.Api_Instance
...
class NearByActivity : AppCompatActivity(),NetworkStateReceiver.NetworkStateReceiverListener,OnMapReadyCallback,LocationDialogCallback {
    ...
    fun convertMarkersListToModel(results: MutableList<Result>) {
    ...
    private fun generateMarkers(markersModel: Result) {
       // markers!!.clear()
            ...
            oMapboxMap.addMarker(
                MarkerOptions().position(LatLng(markersModel.geocodes.main.latitude,markersModel.geocodes.main.longitude)).setSnippet(markerInfoString).icon(icon))

        oMapboxMap.setInfoWindowAdapter(object : MapboxMap.InfoWindowAdapter {
            @Nullable
            override fun getInfoWindow(@NonNull marker: Marker): View {
                val gson = Gson()
                val aMarkerInfo:Result =
                    gson.fromJson(marker.snippet, Result::class.java)
                val v =LayoutInflater.from(this@NearByActivity).inflate(R.layout.custom_info_window_nearby_place, null)
                ...
                Log.d("getInfoWindowTag", "getInfoWindow: "+dataModel!!.nearByCatName+","+dataModel!!.nearByCatIcon)
                UtilsFunctionClass.setImageInGlideFromDrawable(this@NearByActivity
                    ,getDrawable(dataModel!!.nearByCatIcon)!!,myLoading,myImage)
                ...
                myNavBtn.setOnClickListener {
                    if(currentLocation!=null){
                        if(currentLocation!!.latitude!=0.0 && currentLocation!!.longitude!=0.0){
                            ...
                            intent.putExtra("navigation_model",NavigationModel(currentLocation!!.latitude,currentLocation!!.longitude,aMarkerInfo.geocodes.main.latitude,aMarkerInfo.geocodes.main.longitude,0))
                            if (Build.VERSION.SDK_INT<31) {
                                startActivity(intent)
                            }else{
                                Toast.makeText(this@NearByActivity,"Feature Not Available!",Toast.LENGTH_SHORT).show()
